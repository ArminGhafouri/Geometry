...
public:
	...
	int size_row() const { return this->nrows; }
...
{
	if (this != &mat) {
		if (this->nrows != mat.nrows || ncols != mat.ncols) error("bad matrix sizes");
		for (int i = 0; i < this->nrows; i++)
...
FrgBase_FullMtx<T> FrgBase_FullMtx<T>::operator-()
{
	FrgBase_FullMtx<T> zero(this->nrows, ncols);
	return zero - *this;
}
...
{
	if (this->nrows != bb.size() || this->nrows != ncols) error("matrix or vector sizes do not match");
	...
	for (int i = this->nrows - 1; i >= 0; i--) {
		for (int j = i + 1; j < this->nrows; j++) bb[i] -= tmpx[i][j] * bb[j];
		bb[i] /= tmpx[i][i];
	}
...
void FrgBase_FullMtx<T>::GaussElimPP(FrgBase_Vcr<T>& bb) const
{
	...
	FrgBase_Vcr<int> pvt(this->nrows);   // pivoting vector
	for (int k = 0; k < this->nrows; k++) pvt[k] = k;

	for (int k = 0; k < this->nrows - 1; k++) {  // main loop
	...
	FrgBase_Vcr<T> xx(this->nrows);  // xx stores solution in correct order
	for (int i = this->nrows - 1; i >= 0; i--) {
...
void FrgBase_FullMtx<T>::GaussElimCP(FrgBase_Vcr<T>& bb) const
{
	...
	FrgBase_Vcr<int> px(this->nrows);   // row pivoting vector
	FrgBase_Vcr<int> qy(this->nrows);   // column pivoting vector
	...
	FrgBase_Vcr<T> xx(this->nrows);  // xx stores solution
	for (int i = this->nrows - 1; i >= 0; i--) {
